...
export class UploadController {
  ...
  async uploadFile(@UploadedFile() file: Express.Multer.File) {
    try {
      ...
      if (!supabase) {
        console.error(
          'Falha ao inicializar Supabase - verifique SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY',
        );
        throw new HttpException(
          'Configuração de storage ausente no servidor',
      ...
      if (error) {
        ...
        throw new HttpException(
          `Erro ao salvar arquivo no storage: ${error.message}`,
          HttpStatus.INTERNAL_SERVER_ERROR,
        );
      }
      ...
      throw new HttpException(
        `Erro interno no servidor de upload: ${error.message || 'Desconhecido'}`,
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
