...
export class EssaySubmissionController {
  ...
  getSubmissionsByLesson(
    ...
    return this.service.getSubmissionsByLesson(
      lessonId,
      req.user.id,
      req.user.role,
      targetStatus as any,
  ...
  requestRedo(
    ...
    return this.service.requestRedo(
      submissionId,
      req.user.id,
      dto,
      req.user.role,
  ...
  async download(
    ...
    const { fileUrl, prettyName } = await this.service.getDownloadInfo(
      submissionId,
      req.user.id,
      req.user.role,
    );
    ...
    https
      .get(fileUrl, (externalRes: any) => {
        ...
        const encodedName = encodeURIComponent(prettyName)
          .replace(/'/g, '%27')
          .replace(/\(/g, '%28')
          .replace(/\)/g, '%29')
          .replace(/\*/g, '%2A');
