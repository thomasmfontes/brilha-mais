...
export class CourseService {
  ...
  private transformCourse(course: any, userId?: string) {
    ...
    const completedLessons = allLessons.filter(
      (l: any) => l.progress?.length > 0 && l.progress[0].isCompleted,
    );
    const progress =
      totalLessons > 0
      ...
      modules:
        course.modules?.map((mod: any) => ({
          ...
          lessons: mod.lessons?.map((less: any) => ({
            ...less,
            completed:
              less.progress?.length > 0 && less.progress[0].isCompleted,
  ...
  async findAdminCourses(locationId?: string) {
    ...
    return courses.map((course) => ({
      ...
      locationName: course.isGlobal
        ? 'Global'
        : course.location?.name || 'Não definida',
    }));
  }
  ...
  async findByInstructor(instructorId: string, role?: string) {
    ...
    if (isSuperAdmin) {
      ...
      where.OR = [{ locationId: user?.locationId }, { isGlobal: true }];
    } else {
      // Instructor sees:
      // 1. Their own courses
  ...
  async findAll(userId?: string) {
    ...
    if (userId) {
      ...
      if (user && user.role !== 'SUPER_ADMIN') {
        ...
        where.AND = [
          {
            OR: [{ locationId: user.locationId }, { isGlobal: true }],
          },
        ];
          ...
          data: {
            ...
            lessons: {
              create: (mod.lessons || []).map((less: any, lIdx: number) => ({
                ...
                videoUrl: less.youtubeId
                  ? `https://youtube.com/watch?v=${less.youtubeId}`
                  : less.videoUrl,
                pdfUrl: less.pdfUrl,
                description: less.description,
                ...
                quizzes: less.quiz
                  ? {
                      create: {
                        ...
                        questions: {
                          create: (less.quiz.questions || []).map(
                            (q: any, qIdx: number) => ({
                              text: q.text,
  ...
  async getCourseStudentProgress(
    ...
    if (!isAdmin) {
      ...
      const hasLocationAccess =
        course.isGlobal || course.locationId === instructor?.locationId;
    ...
    return modules.map((mod) => ({
      ...
      lessons: mod.lessons.map((lesson) => ({
        ...
        youtubeId: lesson.videoUrl?.includes('youtube.com')
          ? lesson.videoUrl.split('v=')[1]?.split('&')[0]
          : null,
        isCompleted:
          lesson.progress.length > 0 && lesson.progress[0].isCompleted,
        completedAt:
          lesson.progress.length > 0 ? lesson.progress[0].updatedAt : null,
  ...
  async getLessonStudentsProgress(
    courseId: string,
    lessonId: string,
    instructorId: string,
    role?: string,
